...
| **Device-time ordering** | Stored by the device's timestamp, not arrival time, so late/buffered data lands in the right historical slot. | `server/main.py` |
...
| **Server-side alert rules** | Threshold rules over your signals; breaches recorded idempotently. | `server/main.py` |
...
| **Sensor-agnostic core** | The pipeline only moves `metric / value / timestamp`; device specifics live in small integrations. | `sdk/integrations/` |
| **Raspberry Pi system metrics** | Hardware-free integration: streams CPU temp, load, memory, uptime from any Pi. | `sdk/integrations/raspberry_pi.py` |
| **Command-center portal** | Live charts, device health, alerts, rules, device setup — one page, no build step, no CDN. | `dashboard/index.html` |
| **Own backend** | Plain REST to your server: no vendor lock-in, no cloud credentials on the device. | `server/main.py` |
| **Solar-car bridge (flagship)** | One call hands a solar car's decoded `vehicle_state` to the SDK; plus an off-car simulator for demos. | `sdk/integrations/solar_race.py` |
